...
fetch('http://127.0.0.1:8000/api/users/approvedByHOD/')
    .then(response => response.json())
    .then(approvedUsers => {
        const approvedUsernames = approvedUsers.map(user => user.username);

       
        return fetch('http://127.0.0.1:8000/api/formfilled/')
            .then(response => response.json())
            .then(usernames => {
...
function populateUI(apiData) {
    ...
    apiData.forEach(data => {
        ...
        rejectButton.addEventListener('click', function() {
            ...
            if (!formHolder.querySelector('.rejectForm')) {
                ...
                rejectForm.addEventListener('submit', function(event) {
                    ...
                    fetch('http://127.0.0.1:8000/api/users/hodrejected/', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(response => {
                        if (response.ok) {
                            ...
                            return fetch(`http://127.0.0.1:8000/api/users/deleteFormHOD/${useridnumber}/`, {
                                method: 'DELETE',
                                headers: {
                                    'X-CSRFToken': getCookie('csrftoken')
                                }
        ...
        acceptButton.addEventListener('click', function() {
            ...
            fetch('http://127.0.0.1:8000/api/users/hodapproved/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': getCookie('csrftoken')  
